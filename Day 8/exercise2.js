...
const contains = (a, b) => a.split('').filter((el) => b.includes(el)).length === b.length;

const notCommon = (a, b) => a.split('').filter((el) => !b.split('').includes(el)).join('');

for (let i = 0; i < input.length; i++) {
  const [entry, output] = input[i].split(' | ');
  const splitted = entry.split(' ');
  const outputSplitted = output.split(' ');

  // Cases '1', '4', '7' and '8'.
  const one = splitted.filter((el) => el.length === 2).join('');
  const four = splitted.filter((el) => el.length === 4).join('');
  const seven = splitted.filter((el) => el.length === 3).join('');
  const eight = splitted.filter((el) => el.length === 7).join('');
  const nums = [0, one, 0, 0, four, 0, 0, seven, eight, 0];
  for (let j = 0; j < splitted.length; j++) {
    let storeIndex = undefined;
    // Cases '2', '3' and '5'.
    if (splitted[j].length === 5) {
      if (contains(splitted[j], one)) {
        storeIndex = 3;
      } else if (contains(splitted[j], notCommon(four, one))) {
        storeIndex = 5;
      } else {
        storeIndex = 2;
      }
    ...
    // Cases '0', '6' and '9'.
    if (splitted[j].length === 6) {
      if (!contains(splitted[j], one)) {
        storeIndex = 6;
      } else if (contains(splitted[j], four)) {
        storeIndex = 9;
      } else {
        storeIndex = 0;
      }
    ...
    if (storeIndex !== undefined) {
      nums[storeIndex] = splitted[j];
    }
  }

  let val = '';
  outputSplitted.forEach((o) => {
    nums.forEach((num, idx) => {
      if (o && num && (o.split('').sort().join('') === num.split('').sort().join(''))) {
        val += idx;
      }
    });
  });
  occurrences += parseInt(val);
}
